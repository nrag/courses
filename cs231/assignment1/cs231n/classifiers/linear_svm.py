...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  scores = X.dot(W) 
  margin = np.maximum(0, scores - scores[range(num_train), y].reshape(num_train, 1) + 1)
  ...
  #############################################################################
  binmargin = margin
  binmargin[margin > 0] = 1

  indicator = np.zeros(binmargin.shape)
  indicator[range(num_train), y] = 1
  
  dW = np.dot(X.T, binmargin) - np.dot(X.T, indicator * np.sum(binmargin, axis=1).reshape(num_train, 1))
